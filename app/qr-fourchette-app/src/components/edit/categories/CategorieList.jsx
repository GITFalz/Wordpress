...
export default function CategorieList({ user, onMouseDown }) {
    ...
    useEffect(() => {
        ...
        async function fetchCategorie() {
            try {
                setLoading(true);
                const response = await fetch(`${import.meta.env.PUBLIC_API_BASE_URL}/api/categorie/${user.id}`, {
                    headers: {
    ...
    const updateField = async (categorieId, field, value) => {
        const response = await fetch(`${import.meta.env.PUBLIC_API_BASE_URL}/api/categorie/${user.id}/${categorieId}/${field}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem(TOKEN_KEY)}`,
    ...
    const handleDeleteCategorieItem = async (id) => {
        ...
        const response = await fetch(`${import.meta.env.PUBLIC_API_BASE_URL}/api/categorie/${user.id}/${id}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem(TOKEN_KEY)}`,
            },
    ...
    return (
        <div className="w-full edit-info categorie-info">
            <h3 className="text-xl font-semibold text-white bg-orange-400 p-5 rounded-t-xl">VOS PRODUITS</h3>
